...
# example of the string 14/12/2019:05:16:31 -- Robert Bruce Email jonesgregory@example.net 07/08/1978 theft black 1000 true
...
class collectservices(beam.DoFn):
    ...
    def process(self, element):
        ...
        """
        result = [(element['service'], int(element['amount']))]
        return result
...
def main(argv=None):
    ...
    table2 = (csv_lines
              ...
              |'Sum services' >> beam.Map(lambda (services, amount): (service,sum(amount)))
              |'WriteToBigQuery2' >> beam.io.WriteToBigQuery('my-gce-project1:china.services',schema=schema2,
              write_disposition=beam.io.BigQueryDisposition.WRITE_APPEND)
              )
